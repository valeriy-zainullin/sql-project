...
CREATE TABLE Monitors (
  id                   UINT        PRIMARY KEY,
  product_id           UINT        NOT NULL UNIQUE,
...
CREATE TABLE Cpus (
  id                   UINT        PRIMARY KEY,
  vendor               VARCHAR(16) NOT NULL CHECK(vendor = INITCAP(vendor)),
...
CREATE TABLE Monoblocks (
  ...
  FOREIGN KEY (product_id) REFERENCES Products(id) ON DELETE CASCADE,
  FOREIGN KEY (monitor_part_id) REFERENCES Monitors(id) ON DELETE RESTRICT,
